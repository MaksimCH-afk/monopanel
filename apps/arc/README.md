...
| `IA_RATE_LIMIT`                       | 1.0    | Общий троттл req/s ко всему IA (CDX+Availability+снапшоты). ⚠️ С конца 2024 IA держит жёсткий лимит ~60 req/min = 1 req/s на CDX; при игнорировании 429 дольше минуты IP банится на 1 час (с удвоением). Держите ≤1–2, иначе бан. |
...
### Режим «лучший слепок» (отдельный инструмент)

Отдельная вкладка **«Лучший слепок»** в шапке (рядом с «Прогоны») —
страница `/best` со своим вводом доменов и историей поисков. Это
облегчённый инструмент: он НЕ гоняет полный анализ (без LLM-тематики,
редиректов, дропов и вердикта), а только находит **наиболее полную
сохранённую копию главной страницы по каждому календарному году** архива.

- Периоды строятся из таймстемпов CDX (по одному окну на год), а не из
  LLM-эпох — поэтому режим работает даже без LLM-ключа.
- Проверки полноты те же, что и у best-snapshot в полном прогоне: покрытие
  ресурсов (CSS/JS/img) через Availability API + целостность HTML.
- Использует параметры `BEST_SNAPSHOT_*` (`TOP_N`, `MAX_RESOURCES`,
  `PER_EPOCH_TIMEOUT_SEC`, `EPOCH_PARALLELISM`, `MAX_EPOCHS` — ограничивает
  число годовых окон, берутся самые свежие). Флаг `ENABLE_BEST_SNAPSHOT`
  для этого режима не требуется — он включается самой кнопкой.
- Результат виден в карточке домена в блоке «Лучший слепок» (по строке на
  год со ссылкой «открыть»); карточка помечена бейджем «режим: лучший
  слепок». «Повторить прогон» сохраняет режим.
