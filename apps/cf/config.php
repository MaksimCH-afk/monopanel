...
try {
    ...
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_security_rules_user ON security_rules(user_id)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_security_rules_domain ON security_rules(domain_id)");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS app_settings (key TEXT PRIMARY KEY, value TEXT)");
    // Сохранённые мастер-токены (создание/управление child-токенами без повторного ввода).
    $pdo->exec("CREATE TABLE IF NOT EXISTS master_tokens (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        label TEXT, token TEXT, account_email TEXT, domains_hint TEXT,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP
    )");
...
// Освобождаем session-lock СРАЗУ для всех API-эндпоинтов.
// PHP по умолчанию держит эксклюзивную блокировку файла сессии до конца запроса,
// поэтому один медленный запрос (например, probe прав токена или apply с вызовами
// Cloudflare) блокирует ВСЕ параллельные запросы того же браузера на session_start().
// Это и вызывало «бесконечный спиннер»: apply ждал освобождения сессии, а не CF.
// Сессия дальше только читается ($_SESSION['user_id']), писать в неё не нужно.
if ($_isApiEndpoint) {
    session_write_close();
}
